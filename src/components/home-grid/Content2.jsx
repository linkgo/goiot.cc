...
function Content2(props){
	...
	const childrenSource = Object.keys(props.dataSource).filter(key => key.match('block'))
      .map((key, i) => {
      	...
      	return (
      		<Col xs={{ span: 12 }} sm={{ span: 8}} 
	    			...
	    			key={`col${i}`}
	 	   		>
	    			<TweenOne className="content2-list-cell"
	    				animation={{ x: '-=10', opacity: 0, type: 'from' }}
	    				key="img"
	    			>
	    				<img key="img" src={ item.iconImg } />
	    			</TweenOne>
	    			<QueueAnim className="content2-list-cell"
	    				delay={100} leaveReverse key="text"
	    			>
